...
fn parse_string(instructions: &str) -> Vec<(Direction, i32)> {
    instructions.split(',').map(|instruction| {
        let (direction_as_char, digits) = instruction.split_at(1);
        let direction: Direction = if direction_as_char == "U" {
            Direction::Up
        } else if direction_as_char == "D" {
            Direction::Down
        } else if direction_as_char == "R" {
            Direction::Right
        } else if direction_as_char == "L" {
            Direction::Left
        } else {
            panic!("Invalid char");
        };
        (direction, digits.parse::<i32>().unwrap())
    }).collect()
}

fn compute_part1(instructions_1_as_string: &str, instructions_2_as_string: &str) -> i32 {
    let instructions_1 = parse_string(instructions_1_as_string);
    let instructions_2 = parse_string(instructions_2_as_string);
    ...
    let mut intersections: Vec<(i32, i32)> = vec![];
    {
        let mut x = 0;
        let mut y = 0;
        for (direction, quantity) in instructions_1 {
            for _ in 1..=quantity {
                match direction {
                    Direction::Up => y += 1,
                    Direction::Down => y -= 1,
...

    {
        let mut x = 0;
        let mut y = 0;
        for (direction, quantity) in instructions_2 {
            for _ in 1..=quantity {
                match direction {
                    Direction::Up => y += 1,
                    Direction::Down => y -= 1,
    ...
    let mut min_distance = 9_999_999;
...
fn compute_part2(instructions_1_as_string: &str, instructions_2_as_string: &str) -> i32 {
    let instructions_1 = parse_string(instructions_1_as_string);
    let instructions_2 = parse_string(instructions_2_as_string);
    ...
    let mut intersections: Vec<i32> = vec![];
    {
        ...
        for (direction, quantity) in instructions_2 {
            for _ in 1..=quantity {
                ...
                steps += 1;
                if let Some(other_steps) = points.get(&(x, y)) {
                    intersections.push(other_steps + steps);
                }
            }
        }
    }
...
#[test]
fn part1() {
    assert_eq!(159, compute_part1("R75,D30,R83,U83,L12,D49,R71,U7,L72", "U62,R66,U55,R34,D71,R55,D58,R83"));
    assert_eq!(135, compute_part1("R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51", "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7"));
}

#[test]
fn part2() {
    assert_eq!(610, compute_part2("R75,D30,R83,U83,L12,D49,R71,U7,L72", "U62,R66,U55,R34,D71,R55,D58,R83"));
    assert_eq!(410, compute_part2("R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51", "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7"));
}